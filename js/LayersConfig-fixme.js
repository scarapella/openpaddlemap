BR.LayersConfig = L.Class.extend({
    ...
    createMvtLayer(props, options) {
        ...
        if (styleId in BR.layerIndex) {
            // url is key to style in local layers bundle (file name without extension),
            // suggested file naming convention: `<layer id>-style.json` or `<layer id>-style.js`
            options.style = BR.layerIndex[styleId];

            this._replaceMvtTileKey(options.style);
        } else {
            // external URL to style.json or style.js
